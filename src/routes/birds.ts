...
router.get(`${"/"}:id`, async (req, res) => {
	...
	const bird = await req.services.bird.findById(req.params.id);
	if (!bird) {
...
router.post("/", async (req, res) => {
	...
	const createdBird = await req.services.bird.create( {
		name,
		scientificName,
		description,
		imageURL,
	...
	if(!createdBird){
		return res.status(500).send({message: "Error creating bird."});
	}
	res.status(201).send({
...
router.put(`${"/"}:id`, async (req, res) => {
	...
	const replacedBird = await req.services.bird.update(bird);
